export const SHIP_START_POSITION = [0, 0.15, 0]

export const ROTATION_SPEED = 1.35
export const ROTATION_ALIGN_THRESHOLD = 0.035
export const ALIGNMENT_HOLD = 0.4

export const TRAVEL_SPEED = 2.05
export const ARRIVAL_DISTANCE = 2.8

export const CAMERA_FOLLOW_DISTANCE = 7.4
export const CAMERA_HEIGHT = 3.45
export const CAMERA_LOOK_AHEAD = 5.15
export const CAMERA_LERP_SPEED = 2.6
export const CAMERA_DESTINATION_BLEND = 0.16
